...
class CSVExporter:
    ...
    def export_results(
        ...
        if '勝率' in df.columns:
            df['勝率_sort'] = df['勝率'].str.replace('%', '').astype(float)
            df['RR_sort'] = df['RR比'].str.replace('1:', '').astype(float)
            df['リスク_sort'] = df['リスク率'].str.replace('%', '').astype(float)
            df = df.sort_values(['勝率_sort', 'RR_sort', 'リスク_sort'])
            df = df.drop(columns=['勝率_sort', 'RR_sort', 'リスク_sort'])
